...
10. **A patch that is not whole must never look whole.** Any omission — secret paths, oversized/binary untracked files, an absolute-cap cut — sets `diff_complete: false` (+ `diff_truncated` / `original_diff_bytes` for the cap), warns, and leads `next_actions` with "do not apply". `diff_complete` describes the patch, not the transport, so it applies to artifacts identically.
