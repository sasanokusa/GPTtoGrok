...
8. **`response_mode` never changes what is redacted.** The order in `runTool` is fixed: collect → drop secret-path hunks → content redaction → optional absolute cap → measure bytes/hash → pick mode → inline **or** write artifact. `compact` must keep re-using the single redacted string; never re-collect a diff for the artifact, and never write an unredacted diff to a temp file (`src/diff-artifact.ts`, `applyResponseMode` in `src/tools/common.ts`).
